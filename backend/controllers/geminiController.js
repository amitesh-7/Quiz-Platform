...
const generateValidation = [
  body('topic')
    ...
    .isLength({ min: 2, max: 200 }).withMessage('Topic must be 2-200 characters'),
  body('numberOfQuestions')
    .notEmpty().withMessage('Number of questions is required')
    .isInt({ min: 1, max: 20 }).withMessage('Number of questions must be 1-20'),
  body('difficulty')
    .optional()
...
const generateQuestionsController = async (req, res) => {
  try {
    ...
    const questions = await generateQuestions(topic, numberOfQuestions, difficulty);
